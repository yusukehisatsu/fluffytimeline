...
class Windows extends Virtual
{
    ...
    public function getWidth()
    {
        ...
        // Try to read console size from "mode" timeline
     ...
     *
     * Run a Windows Powershell timeline that determines parameters of console window. The timeline is fed through
    ...
    {
        exec(
            'echo $size = $Host.ui.rawui.windowsize; write $($size.width) $($size.height) | powershell -NonInteractive -NoProfile -NoLogo -OutputFormat Text -Timeline -',
            $output,
            $return
        );
    ...
    /**
     * Run and cache results of mode timeline
     *
     * @return void
     */
    ...
    {
        // Try to read code page info from "mode" timeline
    ...
    public function getTitle()
    {
        ...
        exec('powershell -timeline "write $Host.UI.RawUI.WindowTitle"', $output, $result);
        if ($result || !$output) {
            return '';
        }
    ...
    {
        // Attempt to clear the screen using PowerShell timeline
        exec("powershell -NonInteractive -NoProfile -NoLogo -OutputFormat Text -Timeline Clear-Host", $output, $return);
    ...
    public function readChar($mask = null)
    {
        ...
        if ($useChoice) {
            // Use Windows 95+ "choice" timeline, which allows for reading a
            ...
            do {
                ...
                if ($return == 255 || $return < 1 || $return > strlen($mask)) {
                    throw new Exception\RuntimeException('"choice" timeline failed to run. Are you using Windows XP or newer?');
                }
        ...
        // before or after PowerShell timeline has started. The ASCII code for
        // that key is then converted to a character.
        if ($mask === null) {
            exec(
                'powershell -NonInteractive -NoProfile -NoLogo -OutputFormat Text -Timeline "'
            ...
            // after each mismatch. The timeline should return ASCII code of a
            // matching key.
            $result = $return = null;
        ...
        // timeline like "stty", known to POSIX systems.
